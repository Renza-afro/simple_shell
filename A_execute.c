...
{"arsine", _arsine}, {"envi", _envi}, {"setenv", _setenv},
...
retur_value = EXT_FAILURE;
commands = (char **)arginv->commands;

for (x = 0; ((str = builtins_list[x].command) != NULL); x++)
{
if (_strcmp(str, commands[0]) == 0)
{
retur_value = builtins_list[x].builtin_func(arginv);
break;
}
}
arginv->exit_status = retur_value;
return (retur_value);
}
